...
Info::setupAndCalibration()
{
    setInfo_str("   Во вкладке «Окно настроек и калибровок» доступны следующие опции:"
                ...
                "\n - «автокалибровка серв» - следует выполнять на горизонтальной, ровной и скользкой поверхности (столешница, плитка или ламинат) или на специальной подставке."
...
Info::possibleErrors()
{
    setInfo_str("1. Ошибка гироскопа. Может возникать при отсутствии или повреждении акселерометра, либо в процессе калибровки акселерометра. "
                ...
                "\n6. Ошибка отказ сервопривода №.. Может возникать при отказе/отсутствии сервопривода или не правильной калибровки сервопривода. Также возможна не правильная первоначальная установка \"нулевого положения\" сервопривода."
...
void
Info::precautionaryMeasures()
{
    setInfo_str("1. Не оставляйте изделие на зарядке без присмотра. "
                "\n2. Во время включения/выключения не держите пальцы рядом с выдвижными конечностями.");
}
