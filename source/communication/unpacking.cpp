...
void
Unpacking::setRx_commands_Pylt(Rx_commands_Pylt *newRx_commands_Pylt)
{
    _rx_commandsPylt = newRx_commands_Pylt;
}
...
// передача на парс по выбранному устройству
int
Unpacking::choosingDevice()
{
    ...
    else if(_settings->getIdDevice() == _settings->ID_Devices::PYLT)
    {
    ...
    _commDisplay->log_out_S("<- ", "Непредвиденная ошибка распаковки");
    return -7;
}
...
Unpacking::checkID()
{
    ...
    val.data[2] = dataRes[1];
    val.data[1] = dataRes[2];
    val.data[0] = dataRes[3];
    int id = val.int32;

    _commDisplay->log_out_S(tr("<- ID %1").arg(id), "");
    if(_settings->setIdDevice(id, false)) return 1;
    else{
        _commDisplay->log_out_S("<- ", "Неверный ответ на запрос ID");
        return -8;
    }
}
